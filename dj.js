...
const userDance = new Set
const square = [
    {x: 6.5, y: 0.25, z: 22.5},
    {x: 10.5, y: 0.25, z: 22.5},
    {x: 10.5, y: 0.25, z: 28.5},
    {x: 6.5, y: 0.25, z: 28.5}
];
...
const emotes = [
  ...
  { name: "Attentive", id: "idle_layingdown", duration: 23, is_free: false },
...
function getRandomDanceEmote(emotes) {
    const danceEmotes = emotes.filter(emote => {
        const name = emote.name.toLowerCase();
        const id = emote.id.toLowerCase();
        return name.includes('dance') || id.includes('dance');
    });
    
    if (danceEmotes.length === 0) return null;
    
    const randomEmote = danceEmotes[Math.floor(Math.random() * danceEmotes.length)];
    
    return {
        id: randomEmote.id,
        duration: randomEmote.duration,
        start: Date.now()
    };
}
...
setInterval(async () => {
  if (!emote) {
    emote = getRandomDanceEmote(emotes)
    return
  } else {
    if (emote.start + emote.duration*1000 > Date.now()) {
      return
    }
  }
  emote = getRandomDanceEmote(emotes)
...
setInterval(async () => {
  for (const [userID, emoteData] of userEmote) {
    if (userDance.has(userID)) {
      userEmote.delete(userID)
      continue
    }
...
bot.on('playerJoin', async (user, position) => {
  ...
  if (isPointInSquare(position, square)) {
    userDance.add(user.id)
  } else {
    userDance.delete(user.id)
  }
});
...
async function sendPostRequest(name) {
    ...
    try {
        ...
        console.log(`✅ Трек "${name}" успешно отправлен на сервер`);
    } catch (error) {
        // Лучше обрабатывать разные типы ошибок
        if (error.response) {
            // Сервер ответил с кодом вне диапазона 2xx
            console.error('❌ Ошибка ответа сервера:', error.response.status, error.response.data);
        } else if (error.request) {
            // Запрос был сделан, но ответа нет (например, нет связи)
            console.error('❌ Нет ответа от сервера. Проверь подключение к:', error.config.url);
        } else {
            // Остальные ошибки (например, ошибка в настройке запроса)
...
// Тестовые точки
const testPoints = [
    {x: 8.0, z: 25.0},    // Внутри квадрата
    {x: 5.0, z: 25.0},    // Снаружи (слева)
    {x: 12.0, z: 25.0},   // Снаружи (справа)
    {x: 8.0, z: 20.0},    // Снаружи (снизу)
    {x: 8.0, z: 30.0},    // Снаружи (сверху)
    {x: 6.5, z: 22.5},    // На границе (левый нижний угол)
    {x: 10.5, z: 28.5}    // На границе (правый верхний угол)
];

// Проверяем все точки
testPoints.forEach((point, index) => {
    const result = isPointInSquare(point, square);
});
